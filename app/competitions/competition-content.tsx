...
const RoundCard = (round: Round) => {
	const singleDate = round.startDate.getDate() === round.endDate.getDate() && round.startDate.getMonth() === round.endDate.getMonth();
	...
	return (
		<div className="flex flex-row justify-start items-center md:gap-8 flex-shrink-0">
			<div className="bg-gray-200 sm:rounded-md md:rounded-xl sm:w-3/6 md:w-1/6 min-h-48 flex flex-col text-slate-800 text-center sm:px-1 md:px-4 justify-center">
				<h1 className='sm:text-2xl md:text-3xl text-wrap'>{singleDate ? (round.startDate.getDate()) : (`${round.startDate.getDate()}-${round.endDate.getDate()}`)}</h1>
			...
			<div className="md:w-full min-h-48 bg-gray-200 sm:rounded-md md:rounded-xl p-3">
				...
				{round.submissionInstructions && (<><br />
					<p className='text-slate-600'><span className="font-bold">Submission</span>: {round.submissionInstructions}</p></>)}
			</div>
		</div>
	)
}
...
const CompetitionContent = (competition: Competition) => {
	const teamSizeString = competition.teamSize !== undefined ? `${competition.teamSize[0]} - ${competition.teamSize[1]}` : 'Unrestricted'
	const totalPool = competition.pool.reduce((s, v) => s + v.amount, 0);

	return (
		<div className='text-center font-iceland'>
			...
			<div className="space-y-8 font-iceland mt-12 text-base sm:text-lg lg:text-xl text-[#d3dbf0] leading-relaxed text-left mb-4">
				<h1 className="text-4xl text-white circularstd sm:text-5xl lg:text-5xl tracking-tight">Additional Details</h1>
				{competition.additionalDetails && (
					<p>
						{competition.additionalDetails}
					</p>)}
				{competition.rounds.length > 0 && (<>
					<h2 className='text-3xl mt-8 text-white circularstd sm:text-4xl lg:text-4xl tracking-tight'>Rounds</h2>
				...
				{competition.judgingCriteria !== undefined && (<>
					<h2 className='text-3xl mt-8 text-white circularstd sm:text-4xl lg:text-4xl tracking-tight'>Judging Criteria</h2>
					<ul className="list-disc ml-6">
						{competition.judgingCriteria.map((item, index) => {
							if (typeof(item.description) === 'string') {
								return (<li key={index} className='text-xl list-item'>
									{item.title}: {item.description}
								</li>)
							} else {
								return (<li key={index} className='text-xl'>{item.title}:
									<ul className='ml-6'>
										{item.description.map((subitem, subindex) => (<li className="text-xl" key={subindex}>{subitem}</li>))}
									</ul>
								</li>);
							}
							}
				...
				<h2 className='text-3xl mt-8 text-white circularstd sm:text-4xl lg:text-4xl tracking-tight'>Prizes</h2>
				<ul className="list-disc ml-6">
